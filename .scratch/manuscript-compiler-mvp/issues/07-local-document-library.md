...
- [x] 解析 + 编辑后刷新页面，从列表重新打开，Markdown 为最新编辑版
- [x] 文档列表展示各文档（缩略图/时间）并可进入三栏编辑视图
- [x] 删除某文档后，其全部关联文件从本机消失（含附件目录）
- [x] 解析失败的任务不产生半成品文档记录（或可从列表清理）
- [x] 「重新解析」产生的更新覆盖同一 DocumentRecord，不重复建档
