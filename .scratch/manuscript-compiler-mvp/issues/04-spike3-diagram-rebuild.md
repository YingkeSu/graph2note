...
- [x] ≥ 10 张流程图手稿完成 nodes/edges 提取试验，成功率与典型失败模式有记录
- [x] 至少两种绘图方案产出对比样张，给出选型结论（含中文标签与可复现性评价）
- [x] 同一语义两次绘图输出可复现（满足 FR-020 的确定性要求）
- [x] 提取失败样本演示裁剪原图降级路径，无乱码输出
- [x] 结论回写 PRD 开放问题（绘图工具链选型）
