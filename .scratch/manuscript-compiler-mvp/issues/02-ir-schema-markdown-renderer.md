...
- [ ] IR schema 有机器可校验的定义，非法 IR（未知类型、缺字段、非法 JSON）被明确拒绝
- [ ] 渲染为纯确定性函数：同一 IR 两次渲染逐字节一致（回归测试覆盖）
- [ ] golden 测试覆盖全部 10 种 block 类型与边界：空文档、嵌套列表、特殊字符（`*` `#` `_` 反引号）、行内/独立公式、中文全角字符原样保留
- [ ] 不触网、不 mock、可直接在 CI 运行
- [ ] 有一个最小演示入口：IR JSON 文件进、`.md` 出
