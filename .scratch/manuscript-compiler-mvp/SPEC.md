...
- 同页两次扫描光照/角度差异大 → 近重复阈值需可调，误合并可拆分
- PDF 中混入非文档页（空白页、广告页）→ 拆页后按空白/低信息页处理策略（默认保留但标记）
- 缺页预警误报（线索误读，如把普通数字当页码）→ 预警仅为提示，不自动改动文档
- 交叉验证分歧极多（两模型整体不一致）→ 报告按块聚合呈现，不逐条刷屏
- 双模型其一失败/超时 → 降级为单模型结果 + 标注「未验证」，不阻塞出稿
...
- **FR-021**（P1 增补）: 系统 MUST 支持扫描 PDF 上传并按页拆分为图片进入管线，页码可追溯
- **FR-022**（P1 增补）: 系统 MUST 做页级近重复检测（感知哈希）：同页多次扫描/上传合并为同一 DocumentRecord 的候选版本（默认取最新，历史版本可查），MUST NOT 静默新建重复文档
- **FR-023**（P1 增补）: 缺页检测为 best-effort：系统 SHOULD 在检测到线索（页码/日期/系列连续性）时提示疑似缺页；无线索时 MUST NOT 声称完整
- **FR-024**（P1 增补）: 系统 MUST 支持双模型交叉验证：两次独立解析 diff 后产出分歧报告（双侧一致/单侧出现/不一致三类），供用户审核；MUST 支持单文档内近重复块检测
- **FR-025**（P1 增补）: 性能预算：单页端到端（上传→可编辑 Markdown）P95 ≤ 60s（基线实测后可修订）；优化 MUST NOT 使 EditRate 质量回退
...
- **候选版本（Candidate Version，P1 增补）**：同一原始页的多次扫描/解析产物；归属同一 DocumentRecord，默认最新版生效
- **分歧报告（Diff Report，P1 增补）**：双模型交叉验证的产物；按 block 聚合的三类标注（双侧一致/单侧出现/不一致），供用户审核
