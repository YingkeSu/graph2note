...
| workspace | `test_collections.py` `test_document_metadata.py` `test_tags.py` `test_timeline.py` `test_timeline_visual.py` `test_graph.py` `test_graph_layout.py`（U4） `test_graph_interaction.py`（U4 评审修复） `test_inbox.py` `test_telemetry.py` | `-m workspace` | store、webapp |
