...
| 模块 | 测试文件 | 一键 | 契约改动建议连同 |
|---|---|---|---|
| ir | `test_ir.py` `test_versiondiff.py` | `-m ir` | render、pipeline |
...
| webapp | `test_webapp.py` `test_webapp_vault_export.py` `test_webapp_layout.py` `test_webapp_editor_workspace.py` `test_versiondiff_api.py` `test_documents.py` `test_library_cards.py` `test_session_health_cli.py` `test_custom_llm_provider_api.py` | `-m webapp` | pipeline、store |
| notes | `test_vault_exporter.py` `test_incremental_export.py` `test_classify_moc.py` | `-m notes` | store、webapp |
| workspace | `test_collections.py` `test_document_metadata.py` `test_tags.py` `test_timeline.py` `test_timeline_visual.py` `test_graph.py` `test_graph_layout.py`（U4） `test_graph_interaction.py`（U4 评审修复） `test_inbox.py` `test_telemetry.py` `test_collection_organize.py`（auto-organization 02） | `-m workspace` | store、webapp |
