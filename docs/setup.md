...
网关与密钥经环境变量或仓库根 `.env`（gitignored）读取；密钥**不进入**提交与快照。`GRAPH2NOTE_GATEWAY` 选择供应商（`opencode` / `deepseek`，Kimi 通道随后续归档提交加入）。离线契约测试见 `tests/test_gateway_select.py`、`tests/test_llm_settings.py`。
