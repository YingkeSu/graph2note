...
| 4 | `doi` falls back to `full_text` and picks a DOI out of the reference list (`scaling-laws`, `deepseek-moe`) | `metadata._extract_doi` | **fixed (PRR/02 R2–R5)** — DOI only from a front-page metadata line (a metadata marker such as ©/copyright/「Digital Object Identifier」, or a line that is only the label+DOI/URL); citation/reference/body-prose DOIs rejected; a DOI that ends a line followed by another DOI token is **not reconstructed** from plain text (no layout evidence) → empty + `doi-wrap-ambiguous`, while single-line DOIs and DOIs followed by a blank line/paragraph boundary are kept; no minimum-length/digit heuristic (short/alpha-only DOIs such as the DOI Handbook's `10.1000/182` are legitimate) |
