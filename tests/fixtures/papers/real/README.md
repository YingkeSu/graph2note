...
| 4 | `doi` falls back to `full_text` and picks a DOI out of the reference list (`scaling-laws`, `deepseek-moe`) | `metadata._extract_doi` | **fixed (PRR/02 R2–R4)** — DOI only from a front-page metadata line (a metadata marker such as ©/copyright/「Digital Object Identifier」, or a line that is only the label+DOI/URL); citation/reference/body-prose DOIs rejected; a DOI wrapped across a line break is re-joined when the break is unambiguous (digit continuation or after a separator) and **not persisted as a fragment** when ambiguous (`doi-wrap-ambiguous`); no minimum-length/digit heuristic (short/alpha-only DOIs such as the DOI Handbook's `10.1000/182` are legitimate) |
